...
class LocalTrainer:
    ...
    machine learning model artifacts for simulated clients.
    """
    ...
    def _init_model(self) -> BaseModel:
        if self.model_type == "sgd":
            # SGDClassifier baseline - assumes 77 standard numeric features
    ...
    def load_and_split(self, test_size: float = 0.2) -> Tuple[pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray]:
        """Loads client parquet dataset and performs stratified train/test split."""
        ...
        try:
            ...
            logger.warning(f"[{self.client_name}] Stratified split failed (under-represented classes). Falling back to standard split.")
            X_train, X_test, y_train, y_test = train_test_split(
    ...
    def train_and_evaluate(self) -> Dict[str, Any]:
        """Trains model, evaluates metrics, and returns metrics dict."""
        ...
        # Compute multi-class ROC-AUC macro OVR
        try:
        ...
        # Construct comprehensive metrics dict
        metrics_report = {
        ...
        # Save artifacts (model and metrics)
        self.save_artifacts(metrics_report)
        
        return metrics_report

    def save_artifacts(self, metrics_report: Dict[str, Any]):
        """Persists trained model, scaler, and metrics to artifacts/client_name/."""
        ...
        # Save standard scaler artifact
        logger.info(f"[{self.client_name}] Saving scaler to {scaler_path}")
        from sklearn.preprocessing import StandardScaler
        scaler = StandardScaler()
        # Fit a dummy scaler on the preprocessed feature dimensions to ensure shape alignment
    ...
    def load_artifacts(self) -> Tuple[BaseModel, Dict[str, Any]]:
        """Loads serialized model and metrics artifacts from artifacts/client_name/."""
        model_path = self.artifact_dir / "model.pkl"
        metrics_path = self.artifact_dir / "metrics.json"
